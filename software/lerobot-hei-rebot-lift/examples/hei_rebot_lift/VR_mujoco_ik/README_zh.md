...
- 关节指令按真实时间限制速度和加速度；靠近运动学奇异区域时会自动降速。
- IK 解支跳变、目标不可达、进入关节软限位区域或真机关节跟踪误差过大时，只锁住发生异常的那条机械臂。
- 触发时终端会显示 `[HEI VR Safety] ... arm HOLD`。不要继续把手柄推向边界；松开该侧 `grip`，把手柄移离边界，再按住 `grip` 重新建立控制原点。
- 如果真实机械臂已经发生异常运动，应立即急停；重新使能前检查关节反馈、零位标定和电机报警信息。
