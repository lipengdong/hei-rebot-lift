...
DEFAULT_RIGHT_QPOS = np.array([0.0, -0.5, -0.5, 0.0, 0.0, 0.0, GRIPPER_OPEN_QPOS])
DEFAULT_LEFT_QPOS = np.array([0.0, -0.5, -0.5, 0.0, 0.0, 0.0, GRIPPER_OPEN_QPOS])
# IK 限频，默认 60Hz
IK_MAX_RATE_HZ = 60.0
...
class CustomViewer:
    ...
    def show_help(self):
        ...
        print("  - Trigger 扳机: 默认夹爪打开；按下后夹爪关闭，松开后保持当前夹爪状态")
        print("")
        print("特殊功能:")
    ...
    def handle_vr_input(self):
        ...
        # 夹爪控制：默认打开；握住 grip 控制手臂时，trigger 按下关闭、松开打开。
        # 如果松开的是 grip 控制键，则不再刷新夹爪目标，保留松开前的最后状态。
        if right_grip:
            self.right_grip_num = GRIPPER_CLOSED_QPOS if right_trigger else GRIPPER_OPEN_QPOS
        # 设置右手控制器原点
        ...
        # 如果松开的是 grip 控制键，则不再刷新夹爪目标，保留松开前的最后状态。
        if left_grip:
            self.left_grip_num = GRIPPER_CLOSED_QPOS if left_trigger else GRIPPER_OPEN_QPOS
        # 设置左手控制器原点
