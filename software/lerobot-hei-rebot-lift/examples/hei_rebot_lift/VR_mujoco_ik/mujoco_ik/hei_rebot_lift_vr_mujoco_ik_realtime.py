...
# 实时跟手版：在创建 IK solver 前，先设置 IK 内部连续性参数。
# 这里不滤波 VR 的 XYZ/RPY 目标，只约束 IK 关节解不要在相邻帧突然跳支。
# 如果外部已经 export 同名环境变量，则以外部变量为准，方便现场快速调参。
...
# 实时跟手版：不做 IK 限频，但保留很小的静止死区。
# VR 手柄静止时仍会有毫米级/小角度噪声；死区只吃掉这些噪声，避免机械臂原地微抖。
# 如果还抖：把 pos 调到 0.002~0.003；如果觉得细小动作不够灵敏：调回 0.0005。
IK_MAX_RATE_HZ = 0.0
IK_MIN_INTERVAL = 0.0
IK_TARGET_POS_EPS = 0.0012
IK_TARGET_ROT_EPS = np.deg2rad(0.35)
IK_FAIL_LOG_INTERVAL = 1.0
# 实时跟手版：alpha=1.0 表示目标不低通滤波，VR 位姿变化直接进入 IK。
# 如果后续发现手抖被放大，可以先退到 0.8 或 0.7。
...
VIEWER_SYNC_EVERY = 1
# 实时跟手版：主循环不主动 sleep，让 IK/发布尽量跟随当前机器性能跑满。
MAIN_LOOP_SLEEP = 0.0
# 实时跟手版：命令发布不按固定帧率限速；主循环每轮都允许发最新命令。
COMMAND_PUBLISH_RATE_HZ = 0.0
COMMAND_PUBLISH_INTERVAL = 0.0
# A/X 复位时每轮主循环最大关节变化量，单位 rad。
# 只影响“恢复到预设默认姿态”的动作；调小更柔和但回位更慢，调大回位更快但可能更冲。
RESET_MAX_STEP = 0.015
# 实时跟手版：保留轻量关节连续性限幅，专门压运动过程中的 IK 跳解/微小跳变。
# 这不是控制帧率限制；主循环仍跑满，只是每帧关节目标不能突然跨太大。
# 如果还跳：降到 0.05；如果觉得动作被拖慢：升到 0.10~0.12。
...
# 底盘摇杆连续死区：原来 abs(axis)>0.5 才突然启动，容易在阈值附近出现“突突”。
# 现在采用死区后重新映射，摇杆越大速度越连续，低速也更容易保持匀速。
CHASSIS_THUMBSTICK_DEADZONE = 0.15
CHASSIS_MAX_LINEAR_COMMAND = 0.3
CHASSIS_MAX_THETA_COMMAND = 30.0
# 底盘命令轻量平滑：只平滑底盘速度目标，不影响机械臂 XYZ/RPY 实时 1:1 跟手。
# 调大更跟手但更容易抖，调小更稳但启动/停止更柔。
CHASSIS_COMMAND_SMOOTH_ALPHA = 0.35
...
class CustomViewer:
    def __init__(self, model, data, command_endpoint, vr_endpoint):
        ...
        self.chassis_x_cmd = 0.0
        self.chassis_y_cmd = 0.0
        self.chassis_theta_cmd = 0.0
    ...
    def _apply_axis_deadzone(self, value, deadzone):
        ...
        if magnitude <= deadzone:
            return 0.0
        return float(np.sign(value) * (magnitude - deadzone) / max(1.0 - deadzone, 1e-6))

    def _smooth_scalar(self, current, target, alpha):
    ...
    def _publish_robot_command(self):
        ...
        target_theta_vel = 0.0
        if right_grip and right['bButton']:
            target_theta_vel = -CHASSIS_MAX_THETA_COMMAND
        elif right_grip and left['yButton']:
            target_theta_vel = CHASSIS_MAX_THETA_COMMAND

        if right_grip:
            right_thumbstick_y = self._apply_axis_deadzone(right['thumbstick']['y'], CHASSIS_THUMBSTICK_DEADZONE)
            right_thumbstick_x = self._apply_axis_deadzone(right['thumbstick']['x'], CHASSIS_THUMBSTICK_DEADZONE)
            target_x_val = right_thumbstick_y * CHASSIS_MAX_LINEAR_COMMAND
            target_y_val = right_thumbstick_x * CHASSIS_MAX_LINEAR_COMMAND
            self.chassis_x_cmd = self._smooth_scalar(self.chassis_x_cmd, target_x_val, CHASSIS_COMMAND_SMOOTH_ALPHA)
            self.chassis_y_cmd = self._smooth_scalar(self.chassis_y_cmd, target_y_val, CHASSIS_COMMAND_SMOOTH_ALPHA)
            self.chassis_theta_cmd = self._smooth_scalar(
                self.chassis_theta_cmd, target_theta_vel, CHASSIS_COMMAND_SMOOTH_ALPHA
            )
        else:
            # 握把松开必须立即停底盘，不走平滑拖尾。
            self.chassis_x_cmd = 0.0
            self.chassis_y_cmd = 0.0
            self.chassis_theta_cmd = 0.0
        x_val = self.chassis_x_cmd
        y_val = self.chassis_y_cmd
        theta_vel = self.chassis_theta_cmd
